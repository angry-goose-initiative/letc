...
sv2v $VERIF_DIR/include/verif_macros.h $FILELIST $IVERILOG_TOP -DIVERILOG -DREPO_ROOT=$REPO_ROOT --exclude=Assert --exclude=UnbasedUnsized --top=iverilog_top > ${SV2V_OUTPUT} | tee -a $LOG
#sv2v $VERIF_DIR/include/verif_macros.h $FILELIST $IVERILOG_TOP -DIVERILOG -DREPO_ROOT=$REPO_ROOT -DSIMULATION --exclude=Assert --exclude=UnbasedUnsized --top=iverilog_top > ${SV2V_OUTPUT} | tee -a $LOG
